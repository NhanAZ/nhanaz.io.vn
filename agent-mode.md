...
- Read and increment a public page-view total with `GET /api/views?path=/public/path`. This endpoint accepts no credentials and cannot change site content.

## Limits

There is no user account, OAuth flow, MCP server, webhook, content-writing endpoint, checkout, or private data surface. Treat personal memories and criticism as first-person source material, and verify externally checkable claims against the canonical article and its links.
